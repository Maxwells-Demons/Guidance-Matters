...
class StableDiffusionXLPipeline(
    ...
    def reflection_operation(self, i, latents, cfg_gap_list, lora_gap_list, lora_name, lora_scale, prompt_embeds, timesteps, add_text_embeds, add_time_ids, extra_step_kwargs):
        ...
        if cfg_gap_list[0] >= 0:
            ...
            self.v2 = cfg_gap_list[0] * (noise_pred_text - noise_pred_uncond)
            timestep = t
            ...
            self.scale_factor =  ((alpha_prod_t ** 0.5) * (beta_prod_t_prev ** 0.5) - (alpha_prod_t_prev ** 0.5) * (beta_prod_t ** 0.5)) / (alpha_prod_t ** 0.5)
            self.diff_factor = (alpha_prod_t_prev ** 0.5) / (alpha_prod_t ** 0.5)
            self.original_noise_uncond = noise_pred_uncond.clone()
            self.original_noise_text = noise_pred_text.clone()
            self.original_latent = latents.clone()
    ...
    def w2sd_lora(
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                v1 = (latents - self.original_latent * self.diff_factor) / self.scale_factor
                vp = torch.dot(v1.flatten(), self.v2.flatten()) / torch.dot(self.v2.flatten(), self.v2.flatten()) * self.v2
                vv = v1 - vp

                eq_guidance_scale = torch.norm(vp, p=2) / torch.norm(self.original_noise_text - self.original_noise_uncond, p=2)
