...
class StableDiffusionXLPipeline(
        ...
        with self.unet.mid_block.attentions[0].register_forward_hook(get_map_size):
            with self.progress_bar(total=num_inference_steps) as progress_bar:
                for i, t in enumerate(timesteps):
                    if self.interrupt:
                        continue
                    if len(self.guidance_scale_list) == 0 or do_self_attention_guidance:
                    ...
                    if do_self_attention_guidance:
                        ...
                        if self.do_classifier_free_guidance:
                            ...
                            degraded_pred = self.unet(
                                ...
                                # timestep_cond=timestep_cond,
                                # cross_attention_kwargs=self.cross_attention_kwargs,
                                added_cond_kwargs={"text_embeds": add_text_embeds[0].unsqueeze(0), "time_ids": add_time_ids[0].unsqueeze(0)},
                                return_dict=False,
                            )[0]
                            # degraded_pred = self.unet(
                            #     torch.cat([degraded_latents] * 2),
                            #     t,
                            #     encoder_hidden_states=prompt_embeds,
                            #     timestep_cond=timestep_cond,
                            #     cross_attention_kwargs=self.cross_attention_kwargs,
                            ...
                            #     return_dict=False,
                            # )[0]
                            # degraded_pred, _ = degraded_pred.chunk(2)
                            ...
                            vv = v1 - vp
                            eq_guidance_scale1 = torch.norm(vp, p=2) / torch.norm(noise_pred_text - noise_pred_uncond, p=2)
                    ...
                    if do_self_attention_guidance:
                        v1 = (latents - original_latent * diff_factor) / scale_factor
                        vp = torch.dot(v1.flatten(), v2.flatten()) / torch.dot(v2.flatten(), v2.flatten()) * v2
                        vv = v1 - vp

                        eq_guidance_scale = torch.norm(vp, p=2) / torch.norm(original_noise_text - original_noise_uncond, p=2)
                        self.guidance_scale_list.append(max(eq_guidance_scale.item(), eq_guidance_scale1.item()))
    ...
    def pred_x0(self, sample, model_output, timestep):
        alpha_prod_t = self.scheduler.alphas_cumprod[timestep.long().cpu()].to(sample.device)
    ...
    def pred_epsilon(self, sample, model_output, timestep):
        alpha_prod_t = self.scheduler.alphas_cumprod[timestep.long().cpu()]
