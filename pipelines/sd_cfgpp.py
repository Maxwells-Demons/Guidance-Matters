        ...
        **kwargs,
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                next_latent_cfg = self.scheduler.step(noise_pred, t, latents, **extra_step_kwargs, return_dict=False)[0]
                self.scheduler._step_index -= 1
                step_index = self.scheduler.index_for_timestep(t)
                sigma = self.scheduler.sigmas[step_index]
                next_sigma = self.scheduler.sigmas[step_index+1]

                derivative = (next_latent_cfg - cur_latent) / (next_sigma - sigma)
                pred_original_sample = cur_latent - sigma * derivative
                model_output = (pred_original_sample - (cur_latent / (sigma**2 + 1))) / (-sigma / (sigma**2 + 1) ** 0.5)
                v2 = model_output - noise_pred_uncond
                # v2 = (next_latent_cfg - cur_latent) / (next_sigma - sigma) - noise_pred_uncond

                # compute the previous noisy sample x_t -> x_t-1
                latents_dtype = latents.dtype
                latents = self.scheduler.step2(noise_pred_text, noise_pred_uncond, t, latents, **extra_step_kwargs, return_dict=False)[0]
                # latents = self.scheduler.step2(noise_pred, noise_pred, t, latents, **extra_step_kwargs, return_dict=False)[0]
                
                next_latent_cfgpp = latents.clone()
                derivative = (next_latent_cfgpp - cur_latent) / (next_sigma - sigma)
                pred_original_sample = cur_latent - sigma * derivative
                model_output = (pred_original_sample - (cur_latent / (sigma**2 + 1))) / (-sigma / (sigma**2 + 1) ** 0.5)
                v1 = model_output - noise_pred_uncond
                # v1 = (next_latent_cfgpp - cur_latent) / (next_sigma - sigma) - noise_pred_uncond
...
class EulerDiscreteScheduler(SchedulerMixin, ConfigMixin):
    ...
    def step2(
        ...
        if self.config.prediction_type == "epsilon":
            prev_sample = sample - sigma_hat * model_output + self.sigmas[self.step_index+1] * model_output_uncond
        elif self.config.prediction_type == "v_prediction":
            pred_original_sample_uncond = model_output_uncond * (-sigma / (sigma**2 + 1) ** 0.5) + (sample / (sigma**2 + 1))
            derivative_uncond = (sample - pred_original_sample_uncond) / sigma_hat
            prev_sample = sample - sigma_hat * derivative + self.sigmas[self.step_index+1] * derivative_uncond
